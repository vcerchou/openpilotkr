  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.carFingerprint in CANFD_CAR:
          ...
          if not self.gap_by_spd_on or not self.gap_by_spd_on_sw_trg:
            ...
            0 < self.sm['longitudinalPlan'].e2eX[12] < 120 and CS.lead_objspd < -4:
              ...
              if self.switch_timer > 0:
                ...
                can_sends.extend([hyundaican.create_clu11(self.packer, self.resume_cnt, CS.clu11, btn_signal)] * 15) if not self.longcontrol \
                else can_sends.extend([hyundaican.create_clu11(self.packer, self.frame, CS.clu11, btn_signal, clu11_speed, self.CP.sccBus)] * 15)
                self.resume_cnt += 1
                if self.resume_cnt >= randint(6, 8):
                  self.resume_cnt = 0
                  self.switch_timer = randint(30, 36)
      ...
      if CS.out.cruiseState.accActive:
        str_log2 = 'AQ={:+04.2f}  VF={:03.0f}/{:03.0f}  TS={:03.0f}  SS/VS={:03.0f}/{:03.0f}  RD/ED/C/T={:04.1f}/{:04.1f}/{}/{}  C={:1.0f}/{:1.0f}'.format(
