...
void HomeWindow::mousePressEvent(QMouseEvent* e) 
{
  ...
  // OPKR REC
  if (uiState()->scene.started && !sidebar->isVisible() && uiState()->scene.comma_stock_ui != 1 && rec_btn.ptInRect(e->x(), e->y()) && uiState()->scene.mapbox_running) {
    uiState()->scene.touched = true;
    return;
  }
  // Laneless mode
  if (uiState()->scene.started && !sidebar->isVisible() && uiState()->scene.comma_stock_ui != 1 && laneless_btn.ptInRect(e->x(), e->y()) && uiState()->scene.mapbox_running) {
  ...
  // Monitoring mode
  if (uiState()->scene.started && !sidebar->isVisible() && monitoring_btn.ptInRect(e->x(), e->y()) && uiState()->scene.mapbox_running) {
  ...
  // Stock UI Toggle
  if (uiState()->scene.started && !sidebar->isVisible() && stockui_btn.ptInRect(e->x(), e->y())) {
  ...
  if (uiState()->scene.started && !sidebar->isVisible() && speedlimit_btn.ptInRect(e->x(), e->y())) {
