...
class LateralPlanner:
  ...
  def update(self, sm, CP):
    ...
    self.DH.update(sm['carState'], sm['controlsState'], sm['carControl'].latActive, lane_change_prob, md)
    ...
    if self.laneless_mode == 0:
      ...
      yaw_rate_pts = np.interp(v_ego * self.t_idxs[:LAT_MPC_N + 1], np.linalg.norm(self.path_xyz, axis=1), self.plan_curv_rate)
